...
exports.getVerificationRequests = async (req, res) => {
    ...
    try {
        ...
        for(let i = 0; i < user.verificationRequests.length; i++){

            const requestingUser = await User.findById(user.verificationRequests[i])
        ...
        return res.status(200).json({
            success: true,
            verificationRequests: requestingUserData,
            message: "Fetching of verification requests done successfully.."
        })
